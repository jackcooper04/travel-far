  ...
  fs.readFile("src/database.json", "utf8", (err, jsonString) => {
    ...
    try {
      ...
      if (countrySelected.length === 0) {
        var didYouMean = [];
        var tolerance = 4;
        fs.readFile("src/database.json", "utf8", (err, result) => {
          var smalldatabase = JSON.parse(result);
          smalldatabase.forEach(function (small) {
            var levenshteinValue = levenshtein(input, small.name);
            if (levenshteinValue <= tolerance) {
              didYouMean.push(small.name);
            }
          });
          console.log(didYouMean);
          if (didYouMean.length != 0) {
            var alertString = didYouMean.toString();
            alert("Did you mean " + alertString + " ?");
          } else {
            alert("Country Not Found!");
          }
        });
      } else {
        console.log(countrySelected[0]);
        document.getElementById("search").innerHTML =
          `<h1>` +
          countrySelected[0].name +
          `</h1>
        <h2>Price: $` +
          countrySelected[0].price +
          `</h2>`;
      }
    } catch (err) {
...
function viewAllDestinations() {
  ...
  fs.readFile("src/smalldatabase.json", "utf8", (err, jsonString) => {
    ...
    console.log(bubbleSort(parsedData));
  });
}
...
function bubbleSort(inputArr) {
  ...
  do {
    ...
    for (let j = 0; j < len; j++) {
      ...
      inputArr[j].price = parseInt(inputArr[j].price);
      console.log(inputArr[j]);
    }
    console.log(inputArr);
    for (let i = 0; i < len; i++) {
      if ( inputArr[i + 1] && inputArr[i].price > inputArr[i + 1].price) {
        let tmp = inputArr[i].price;
        inputArr[i] = inputArr[i + 1].price;
        inputArr[i + 1].price = tmp;
        swapped = true;
      }
    }
  } while (swapped);
